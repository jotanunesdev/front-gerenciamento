/*
  ...
  This script translates the Supabase/Postgres database layer to SQL Server 2022.
  It creates:
    ...
    - SQL Server Row-Level Security policies using SESSION_CONTEXT

  Session context conventions used by the translated RLS:
...
GRANT EXECUTE ON [dbo].[has_role] TO [authenticated];
GRANT EXECUTE ON [dbo].[has_module] TO [authenticated];
GRANT EXECUTE ON [dbo].[has_role] TO [service_role];
GRANT EXECUTE ON [dbo].[has_module] TO [service_role];
GO
...
/*
  ...
  The current application still depends on Supabase Auth/Admin APIs in the
  TypeScript backend. This script recreates the database layer in SQL Server,
  but the app will need backend refactoring before it can run against SQL Server
  end-to-end.
*/
